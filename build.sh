...
IOSBuild() {
  ...
  $go build -ldflags "-X main.Version=$version -s -w" -o "armv7" github.com/zjdy/baidupcs-server
  jtool --sign --inplace --ent ../../entitlements.xml "armv7"
  export GOARCH=arm64
  $go build -ldflags "-X main.Version=$version -s -w" -o "arm64" github.com/zjdy/baidupcs-server
...
RicePack() {
  return # 已取消web功能
  rice -i github.com/zjdy/baidupcs-server/internal/pcsweb append --exec "$output/$1/$2"
}
